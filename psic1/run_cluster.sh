...
#gmx mdrun -ntmpi $ntmpi -ntomp $ntomp -gpu_id $gpu_id -v -deffnm $em >& log_energy_minimize_mdrun

#-----NVT equilibration-----#
#gmx grompp -f "nvt.mdp" -c $em$gro -r $em$gro -p $topol$frame_no$top -o $nvt$tpr >& log_nvt_grompp

#gmx mdrun -ntmpi $ntmpi -ntomp $ntomp -gpu_id $gpu_id -v -deffnm $nvt >& log_nvt_mdrun

#-----NPT equilibration-----#
gmx grompp -f "npt.mdp" -c $nvt$gro -r $nvt$gro -t $nvt$cpt -p $topol$frame_no$top -o $npt$tpr >& log_npt_grompp

gmx mdrun -ntmpi $ntmpi -ntomp $ntomp -gpu_id $gpu_id -v -deffnm $npt >& log_npt_mdrun

#-----Production MD-----#
gmx grompp -f "md.mdp" -c $npt$gro -t $npt$cpt -p $topol$frame_no$top -o $pmd$tpr >& log_pmd_grompp

gmx mdrun -ntmpi $ntmpi -ntomp $ntomp -gpu_id $gpu_id -v -deffnm $pmd >& log_pmd_mdrun
